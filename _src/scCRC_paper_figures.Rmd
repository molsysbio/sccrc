...
cols.use <- c(
  p007 = palette_OkabeIto[1],
  p008 = palette_OkabeIto[2],
  p009 = palette_OkabeIto[3],
  p012 = palette_OkabeIto[4],
  p013 = palette_OkabeIto[5],
  p014 = palette_OkabeIto[6],
  p016 = palette_OkabeIto[7],
  p017 = "#451077",
  ...
  Stem = magma(10)[9],
  TA = magma(10)[7],
  `Secretory TA` = magma(10)[6],
  `Imma. Ent.` = viridis(6)[5],
  `Enterocytes` = palette_OkabeIto[3],
  `Imma. Gob.` = magma(10)[5],
  Goblet = magma(10)[3],
  `M cells` = palette_OkabeIto[2],
  Tuft = viridis(6)[6],
  `NA` = "grey",
  `Cycling B` = magma(10)[9],
  `B cells` = magma(10)[9],
  Plasma = magma(10)[7],
  Follicular = magma(10)[5],
  GCs = magma(10)[3],
  Treg = palette_OkabeIto[5],
  Tconv = palette_OkabeIto[2],
  `CD4+ T` = palette_OkabeIto[6],
  `CD8+ T` = palette_OkabeIto[7],  
  `Mast` = palette_OkabeIto[6],
  DC1 = palette_OkabeIto[3],
  DC2 = palette_OkabeIto[4],
  ILCs = palette_OkabeIto[5],
  Monocytes = palette_OkabeIto[4],
  `Macroph.` = palette_OkabeIto[3],
  `Crypt FBs` = magma(10)[9],
  `Villus FBs` = magma(10)[7],
  `Infl. FBs` = palette_OkabeIto[2],
  `Myofib.` = palette_OkabeIto[3],
  Endothelial = palette_OkabeIto[4],
  `PC venules` = palette_OkabeIto[5],
  `Pericytes` = palette_OkabeIto[6],
  `Glia` = palette_OkabeIto[7], 
  normal = "steelblue",
  tumor = "red",
  PDO = "steelblue"
)
...
poi <- c("p007", "p008", "p009", "p012", "p013", "p014", "p016", "p017")

panel_pws <- frame_data(
  ~Gene, ~Pathway,
  "ABL1", "MAPK/Pi3K",
  "APC", "Wnt/RB1/Notch",
  "ATM", "p53/DDS/NFkB",
...
panelseq_tbl <- lapply(1:8, function(x) read_excel("_data/panelseq_patients.xlsx", x)) %>% 
  set_names(poi) %>% 
  bind_rows(.id = "source_id") %>% 
  filter(is.na(`mut Effect`) | `mut Effect` != "Polymorphismus") %>% 
  select(source_id, Gene, Type, `mut Effect`) %>% 
  bind_rows(tibble(source_id = poi)) %>% 
  complete(source_id, Gene) %>% 
  filter(!is.na(Gene)) %>% 
  mutate(Type = c(`C` = "MUT", `I (Dup)` = "Ins")[Type]) %>% 
  left_join(panel_pws, by = "Gene")

ggplot(panelseq_tbl, aes(source_id, Gene, fill = Type)) +
  geom_tile(aes(Gene, source_id, fill = Type)) +
  geom_tile(aes(Gene, source_id), fill = NA, color = "black") +
...
seu_epi_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_epi_p007n_strict_epi_p007t_strict_epi_p008n_strict_epi_p008t_strict_epi_p009n1_strict_epi_p009n2_strict_epi_p009t1_strict_epi_p009t2_strict_epi_p012n_strict_epi_p012t_strict_epi_p013n_strict_epi_p013t.rds")

seu_markers_epi_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_epi_p007n_strict_epi_p007t_strict_epi_p008n_strict_epi_p008t_strict_epi_p009n1_strict_epi_p009n2_strict_epi_p009t1_strict_epi_p009t2_strict_epi_p012n_strict_epi_p012t_strict_epi_p013n_strict_epi_p013t.rds_markers.rds")

seu_imm_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_imm_p007n_strict_imm_p007t_strict_imm_p008n_strict_imm_p008t_strict_imm_p009n1_strict_imm_p009n2_strict_imm_p009t1_strict_imm_p009t2_strict_imm_p012n_strict_imm_p012t_strict_imm_p013n_strict_imm_p013t.rds")

seu_markers_imm_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_imm_p007n_strict_imm_p007t_strict_imm_p008n_strict_imm_p008t_strict_imm_p009n1_strict_imm_p009n2_strict_imm_p009t1_strict_imm_p009t2_strict_imm_p012n_strict_imm_p012t_strict_imm_p013n_strict_imm_p013t.rds_markers.rds")
                       
seu_str_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_str_p007n_strict_str_p007t_strict_str_p008n_strict_str_p008t_strict_str_p009n1_strict_str_p009n2_strict_str_p009t1_strict_str_p009t2_strict_str_p012n_strict_str_p012t_strict_str_p013n_strict_str_p013t.rds")

seu_markers_str_pri <- read_rds("_data/_patients/computed/annotated/seu_merge_str_p007n_strict_str_p007t_strict_str_p008n_strict_str_p008t_strict_str_p009n1_strict_str_p009n2_strict_str_p009t1_strict_str_p009t2_strict_str_p012n_strict_str_p012t_strict_str_p013n_strict_str_p013t.rds_markers.rds")
...
epi_custom_anno <- frame_data(
  ~SCT_snn_res.1, ~cell_type_epi_custom, ~epi_marker_genes, ~cell_type_epi_color,
  "3", "Stem/TA", "OLFM4", "#00CCFF",
  "2", "TA", "MKI67", "#8080CC",
  "1", "Ent. Prog. 1", "FABP1", "#999900",
  "11", "Ent. Prog. 2", "FABP1", "#CCCC80",
  "8", "Enterocytes 1", "FABP1", "#336600",
  "4", "Enterocytes 2", "FABP1", "#668C40",
  "15", "Enterocytes 3", "FABP1", "#99B380",
  "9", "Enterocytes 4", "FABP1", "#CCD9BF",
  "5", "Imma. Gob. 1", "TFF3", "#990099",
  "0", "Imma. Gob. 2", "TFF3", "#CC80CC",
  "7", "Goblet", "TFF3", "#660099",
  "18", "Tuft", "TRPM5", "#FF9900",
  "13", "TC1", "-", "#AA0000",
  "16", "TC2", "-", "#FF0000",
  "12", "TC3", "-", "#FF0099",
  "6", "TC4", "-", "#EB9999",
  "10", "TC5", "-", "#F5CCCC",
  "14", "Unassigned epithelial 1", "MKI67", "#8080CC",
  "17", "Unassigned epithelial 2", "MKI67", "#CCCCEB"
) %>% 
...
seu_epi_pri@meta.data <- seu_epi_pri@meta.data %>%
  as_tibble %>%
  left_join(epi_custom_anno, by = "SCT_snn_res.1") %>%
  as.data.frame() %>%
...
imm_custom_anno <- frame_data(
  ~SCT_snn_res.1, ~cell_type_imm_custom, ~imm_marker_genes,
  "8", "B cells 1", "MS4A1", 
  "6", "B cells 2", "MS4A1",
  "24", "B cells 3", "MS4A1",
  "23", "B cells 4", "MS4A1",
  ...
  "4", "Monocytes", "CD68",
  "12", "Macroph.", "C1QA",
  "26", "Unassigned immune", "KIT"
) %>% 
  mutate(cell_type_imm_simple = str_remove_all(cell_type_imm_custom, " [0-9]|0")) %>% 
...
seu_imm_pri@meta.data <- seu_imm_pri@meta.data %>%
  as_tibble %>%
  left_join(imm_custom_anno, by = "SCT_snn_res.1") %>%
  as.data.frame() %>%
  set_rownames(.$cell_id)
...
str_custom_anno <- frame_data(
  ~SCT_snn_res.1, ~cell_type_str_custom, ~str_marker_genes, ~cell_type_str_color,
  "8", "CB FBs 1", "-", "#56b4e9",
  "2", "CB FBs 2", "-", "#2772b2",
  "7", "UC FBs", "-", "#fac98d",
  "4", "CCL FBs", "-", "#f1605d",
  "1", "MyoFBs", "-", "#319e73",
  "11", "DES+ MyoFBs", "-", "#65B696",
  "0", "CAFs", "-", "#d55e00",
  "6", "Pericytes", "-", "#FF66FF",
  "5", "Endothelial 1", "-", "#f0e441",
  "3", "Endothelial 2", "-", "#B4AB31",
  "9", "Glia", "-", "#cc79a7",
  "10", "Unassigned stromal", "-", "#FFFFFF"
) %>%
...
seu_str_pri@meta.data <- seu_str_pri@meta.data %>%
  as_tibble %>%
  left_join(str_custom_anno, by = "SCT_snn_res.1") %>%
  as.data.frame() %>%
  set_rownames(.$cell_id)
...
f1ab_tbl <- bind_rows(
  as_tibble(FetchData(seu_epi_pri, c("sample_origin", "source_id", "UMAP_1", "UMAP_2", "main_cell_type", "cell_type_epi_custom", "is_tumor_cell", "nSNPs_normal", "nSNPs_tumor", "Phase"))),
  # as_tibble(FetchData(seu_imm_pri, c("sample_origin", "source_id", "UMAP_1", "UMAP_2", "main_cell_type", "cell_type_imm"))),
  as_tibble(FetchData(seu_imm_pri, c("sample_origin", "source_id", "UMAP_1", "UMAP_2", "main_cell_type", "cell_type_imm_simple", "is_tumor_cell", "nSNPs_normal", "nSNPs_tumor", "Phase"))),    
  as_tibble(FetchData(seu_str_pri, c("sample_origin", "source_id", "UMAP_1", "UMAP_2", "main_cell_type", "cell_type_str_custom", "is_tumor_cell", "nSNPs_normal", "nSNPs_tumor", "Phase")))
) %>% 
  mutate(cell_type_epi_custom = ordered(cell_type_epi_custom, levels = unique(epi_custom_anno$cell_type_epi_custom))) %>% 
  mutate(cell_type_imm_custom = ordered(cell_type_imm_simple, levels = unique(imm_custom_anno$cell_type_imm_simple))) %>% 
...
p4 <- ggplot(filter(f1ab_tbl, main_cell_type == "Immune")) +
  geom_point(aes(UMAP_1, UMAP_2, color = cell_type_imm_simple, size = main_cell_type)) +
  facet_grid(sample_origin~main_cell_type, switch = "y") +
  scale_size_manual(values = c(0.01), guide = "none") +
...
comp_tbl <- f1ab_tbl %>% 
  group_by(sample_origin, main_cell_type, 
           cell_type_epi_custom, cell_type_imm_simple, cell_type_str_custom) %>% 
  tally %>% 
  group_by(sample_origin, main_cell_type) %>% 
...
comp_tbl_tc1 <- f1ab_tbl %>% 
  filter(main_cell_type == "Epithelial",
         !(cell_type_epi_custom %in% c("TC1", "TC2", "TC3", "TC4", "TC5"))) %>%
...
c2 <- comp_plot_base(comp_tbl, "Immune") +
  geom_bar(aes(fct_rev(sample_origin), nrel, fill = fct_rev(cell_type_imm_simple)), stat = "identity") +
...
c2pp <- comp_plot_base(comp_tbl_pp, "Immune") +
  ...
  scale_fill_manual(values = cols.use, guide = guide_legend(ncol = 2, reverse = T), na.value = "grey")
...
ggsave("_fig/_patients/f1_sup_comp_pp_strict.pdf", width = 8, height = 5.5)
            
plot_grid(plot_grid(ggdraw(), ggdraw(), nrow = 1, labels = c("A", "B")),
          plot_grid(c1tc, c2tc, nrow = 1, align = "hv"),
          rel_heights = c(0.05, 0.95), ncol = 1)

ggsave("_fig/_patients/f1_sup_comp_tc_strict.pdf", width = 5, height = 5.5)

c3tc
ggsave("_fig/_patients/f2d_TC_cell_phase.pdf", width = 2, height = 4)
...
ggsave("_fig/_patients/f1abcd_strict.png", width = 10, height = 15)
ggsave("_fig/_patients/f1abcd_strict.pdf", width = 10, height = 15)
...
f1ab_tbl %>% 
  ...
  ggplot(aes(UMAP_1, UMAP_2, color = is_tumor_cell)) +
  geom_point(size = 0.2) +
  coord_fixed() +
  theme_void() +
...
write_tsv(t1, "_data/cell_type_numbers_1.tsv")
write_tsv(t2, "_data/cell_type_numbers_2.tsv")
write_tsv(t3, "_data/cell_type_numbers_3.tsv")
write_tsv(t4, "_data/cell_type_numbers_4.tsv")
...
t4 %>% 
  ...
  spread(sample_id, n) %>% 
  write_tsv("_data/cell_type_numbers_epi.tsv")
...
t4 %>% 
  ...
  write_tsv("_data/cell_type_numbers_epi_relative.tsv")

t4 %>% 
  filter(!is.na(cell_type_imm_simple)) %>% 
  mutate(sample_id = paste0(source_id, "_", sample_origin)) %>% 
  ungroup %>%
  dplyr::select(sample_id, cell_type_imm_simple, nrel) %>% 
  spread(sample_id, nrel) %>% 
  write_tsv("_data/cell_type_numbers_imm_relative.tsv")
...
t4 %>% 
  ...
  spread(sample_id, n) %>% 
  write_tsv("_data/cell_type_numbers_str.tsv")
...
t4 %>% 
  ...
  spread(sample_id, nrel) %>% 
  write_tsv("_data/cell_type_numbers_str_relative.tsv")
    
write_tsv(comp_tbl_tc3, "_data/TC_cell_phase_numbers.tsv")
...
seu_markers_epi_pri %>% 
  ...
  write_tsv("_data/cell_type_markers_epi.tsv")
...
seu_markers_imm_pri %>% 
  ...
  write_tsv("_data/cell_type_markers_imm.tsv")
...
seu_markers_str_pri %>% 
  ...
  write_tsv("_data/cell_type_markers_str.tsv")
...
var_features <- unique(c(rownames(seu_epi_pri@assays$SCT@scale.data), VariableFeatures(seu_epi_pri)))
...
Idents(seu_imm_pri) <- seu_imm_pri$cell_type_imm_simple
imm_markers <- parallel::mclapply(unique(seu_imm_pri$cell_type_imm_simple), function(x) FindMarkers(seu_imm_pri, ident.1 = "tumor", ident.2 = "normal", group.by = "sample_origin", subset.ident = x, logfc.threshold = 0, min.diff.pct = 0, features = var_features), mc.cores = parallel::detectCores()) %>% 
  lapply(error_handling) %>% 
  setNames(unique(seu_imm_pri$cell_type_imm_simple)) %>% 
  lapply(as_tibble, rownames = "gene") %>% 
  bind_rows(.id = "cell_type_imm_simple")

Idents(seu_str_pri) <- seu_str_pri$cell_type_str_custom
str_markers <- parallel::mclapply(unique(seu_str_pri$cell_type_str_custom), function(x) FindMarkers(seu_str_pri, ident.1 = "tumor", ident.2 = "normal", group.by = "sample_origin", subset.ident = x, logfc.threshold = 0, min.diff.pct = 0, features = var_features), mc.cores = parallel::detectCores()) %>% 
  lapply(error_handling) %>% 
...
imm_top_genes <- imm_markers %>% 
  ...
  filter(abs(avg_logFC) > 0.75, p_val_adj < 0.01, 
         !str_detect(gene, "^MT|^HSP|^DNAJ")) %>%
  mutate(cell_type_imm_simple = ordered(cell_type_imm_simple, levels = unique(imm_custom_anno$cell_type_imm_simple))) %>% 
  group_by(gene) %>% 
  mutate(cell_type_rank = row_number(-abs(avg_logFC))) %>% 
  ungroup %>%
  filter(cell_type_rank == 1) %>% 
  mutate(cell_type_rank = ifelse(avg_logFC > 0, 1, -1)) %>%
  arrange(-cell_type_rank, cell_type_imm_simple) %$% 
  unique(gene)
...
imm_markers %>% 
  ...
  ggplot(aes(gene, cell_type_imm_simple, fill = avg_logFC)) +
  geom_tile() +
  scale_fill_gradient2(low = "steelblue", high = "red", mid = "white", midpoint = 0, na.value = "grey50") +
  theme_cowplot2() +
  coord_fixed() +
...
ggsave("_fig/_patients/diff_genes_imm_tn.pdf", width = 8, height = 3)
imm_markers %>% 
  filter(gene %in% imm_top_genes) %>% 
  mutate(cell_type_imm_simple = ordered(cell_type_imm_simple, levels = unique(imm_custom_anno$cell_type_imm_simple))) %>% 
  arrange(cell_type_imm_simple) %>% 
  write_tsv("_data/diff_markers_imm.tsv")
...
ggsave("_fig/_patients/diff_genes_str_tn.pdf", width = 14, height = 3)
str_markers %>% 
  filter(gene %in% str_top_genes) %>% 
  mutate(cell_type_str_custom = ordered(cell_type_str_custom, levels = unique(str_custom_anno$cell_type_str_custom))) %>% 
  arrange(cell_type_str_custom) %>% 
  write_tsv("_data/diff_markers_str.tsv")
...
f1de_tbl <- bind_rows(fd_wraper(seu_epi_pri), fd_wraper(seu_str_pri), fd_wraper(seu_imm_pri))
...
main_grid <- plot_grid(
  ...
  ggdraw(),
  plot_grid(phase_comp_plot, p3, ncol = 2, rel_widths = c(0.3, 0.7)),
  ncol = 1, rel_heights = c(0.49, 0.02, 0.49)
)

panel1 <- ggdraw() + 
  draw_plot(main_grid, 0, 0, 1, 1) +
  draw_plot_label(c("C", "D"), c(0, 0.3), c(0.53, 0.53))
...
ggsave("_fig/_patients/f2abcd_strict_epi.png", width = 16, height = 8)
ggsave("_fig/_patients/f2abcd_strict_epi.pdf", width = 16, height = 8)

cms_plot_list_epi$p2
ggsave("_fig/_patients/f2_sub_strict_epi.png", width = 16, height = 8)
ggsave("_fig/_patients/f2_sub_strict_epi.pdf", width = 16, height = 8)

cms_plot_list_str <- cms_plot_wrapper("Stromal")

cms_plot_list_str$p1
ggsave("_fig/_patients/f2abcd_strict_str.png", width = 16, height = 8)
ggsave("_fig/_patients/f2abcd_strict_str.pdf", width = 16, height = 8)

cms_plot_list_str$p2
ggsave("_fig/_patients/f2_sub_strict_str.png", width = 16, height = 8)
ggsave("_fig/_patients/f2_sub_strict_str.pdf", width = 16, height = 8)

cms_plot_list_imm <- cms_plot_wrapper("Immune")

cms_plot_list_imm$p1
ggsave("_fig/_patients/f2abcd_strict_imm.png", width = 16, height = 8)
ggsave("_fig/_patients/f2abcd_strict_imm.pdf", width = 16, height = 8)

cms_plot_list_imm$p2
ggsave("_fig/_patients/f2_sub_strict_imm.png", width = 16, height = 8)
ggsave("_fig/_patients/f2_sub_strict_imm.pdf", width = 16, height = 8)
...
str_markers <- read_tsv("_data/cell_type_markers_str.tsv") %>% 
  group_by(cell_type_str) %>% 
  slice(1) %$%
  gene

epi_markers_tbl <- read_tsv("_data/cell_type_markers_epi.tsv")
...
tc_epi <- plot_wrapper(seu_epi_pri, c("MMP7", "S100A11", "VIM", "S100A4"), by.case = T, pt.size = 0.01)

m_epi$p1
ggsave("_fig/_patients/f1_markers_epi.png", width = 11.5, height = 4)

m_imm$p1
ggsave("_fig/_patients/f1_markers_imm.png", width = 20, height = 4)

m_str$p1
ggsave("_fig/_patients/f1_markers_str.png", width = 28, height = 4)

pw_epi$p1
ggsave("_fig/_patients/f1_pathways_epi.png", width = 12, height = 4)

pw_imm$p1
ggsave("_fig/_patients/f1_pathways_imm.png", width = 12, height = 4)

pw_str$p1
ggsave("_fig/_patients/f1_pathways_str.png", width = 12, height = 4)
            
tc_epi$p1
ggsave("_fig/_patients/f1_markers_tc.png", width = 12, height = 4)
...
flr_tbl <- bind_rows(
    gather(FetchData(seu_epi_pri, c(paste0("sct_", lr_tbl$Gene), "main_cell_type", "source_id", "sample_origin", "cell_type_epi_custom", "nCount_RNA")), Gene, value, -main_cell_type, -source_id, -sample_origin, -cell_type_epi_custom, -nCount_RNA),
    gather(FetchData(seu_str_pri, c(paste0("sct_", lr_tbl$Gene), "main_cell_type", "source_id", "sample_origin", "cell_type_str_custom", "nCount_RNA")), Gene, value, -main_cell_type, -source_id, -sample_origin, -cell_type_str_custom, -nCount_RNA),
    gather(FetchData(seu_imm_pri, c(paste0("sct_", lr_tbl$Gene), "main_cell_type", "source_id", "sample_origin", "cell_type_imm_simple", "nCount_RNA")), Gene, value, -main_cell_type, -source_id, -sample_origin, -cell_type_imm_simple, -nCount_RNA)
) %>% 
...
lr_top_imm_tbl <- flr_tbl %>% 
  ...
  group_by(cell_type_imm_simple, Gene, key, Pathway, sample_origin) %>% 
  lr_top_wrapper

write_tsv(lr_top_epi_tbl, "_data/_patients/tab/ligand_receptor_top_epi.tsv")
write_tsv(lr_top_str_tbl, "_data/_patients/tab/ligand_receptor_top_str.tsv")
write_tsv(lr_top_imm_tbl, "_data/_patients/tab/ligand_receptor_top_imm.tsv")

lr_top_epi_lig <- c(unique(filter(lr_top_epi_tbl, key == "ligands")$Gene)[1:10], "RSPO3", "HGF")
lr_top_epi_rec <- unique(filter(lr_top_epi_tbl, key == "receptors")$Gene)[1:10]
lr_top_str_lig <- c(unique(filter(lr_top_str_tbl, key == "ligands")$Gene)[1:10], "RSPO3", "HGF")
lr_top_str_rec <- unique(filter(lr_top_str_tbl, key == "receptors")$Gene)[1:10]
lr_top_imm_lig <- c(unique(filter(lr_top_imm_tbl, key == "ligands")$Gene)[1:10], "RSPO3")
...
ggplot(filter(lr_top_epi_tbl, key == "receptors"), aes(cell_type_epi_custom, Gene, fill = tumor)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Tumor")
ggsave("_fig/_patients/f3_epi_receptors_exprs_tumor.pdf", width = 6, height = 16)
...
ggplot(filter(lr_top_str_tbl, key == "ligands"), aes(cell_type_str_custom, Gene, fill = tumor)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Tumor")
ggsave("_fig/_patients/f3_str_ligands_exprs_tumor.pdf", width = 5, height = 16)

ggplot(filter(lr_top_imm_tbl, key == "ligands"), aes(cell_type_imm_simple, Gene, fill = tumor)) +
  geom_tile() +
  scale_fill_gradient2(low = "steelblue", high = "red", mid = "white", midpoint = 0, na.value = "grey50") +
  facet_grid(Pathway~., scales = "free", space = "free") +
  theme_cowplot2() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Tumor")
ggsave("_fig/_patients/f3_imm_ligands_exprs_tumor.pdf", width = 4, height = 16)
...
ggplot(filter(lr_top_epi_tbl, key == "receptors"), aes(cell_type_epi_custom, Gene, fill = normal)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Normal")
ggsave("_fig/_patients/f3_epi_receptors_exprs_normal.pdf", width = 6, height = 16)
...
ggplot(filter(lr_top_str_tbl, key == "ligands"), aes(cell_type_str_custom, Gene, fill = normal)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Normal")
ggsave("_fig/_patients/f3_str_ligands_exprs_normal.pdf", width = 5, height = 16)

ggplot(filter(lr_top_imm_tbl, key == "ligands"), aes(cell_type_imm_simple, Gene, fill = normal)) +
  geom_tile() +
  scale_fill_gradient2(low = "steelblue", high = "red", mid = "white", midpoint = 0, na.value = "grey50") +
  facet_grid(Pathway~., scales = "free", space = "free") +
  theme_cowplot2() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  labs(fill = "Avg. Exprs.\nin Tumor")
ggsave("_fig/_patients/f3_imm_ligands_exprs_normal.pdf", width = 4, height = 16)
...
flr_tbl_summary <- flr_tbl %>% 
  mutate(sub_cell_type = ifelse(main_cell_type == "Epithelial", as.character(cell_type_epi_custom), ifelse(main_cell_type == "Immune", as.character(cell_type_imm_simple), as.character(cell_type_str_custom)))) %>%
  mutate(rel_value = value/log(nCount_RNA+1)) %>% 
...
imm_cell_type_changes <- t4 %>%
  filter(main_cell_type == "Immune") %>% 
  group_by(sample_origin, cell_type_imm_simple) %>% 
  summarise(n = sum(n)) %>% 
  replace_na(list(n = 0)) %>% 
  select(sub_cell_type.x = cell_type_imm_simple, sample_origin, n_imm_cells = n)
...
ligands_str <- ligand_receptor_summary %>% 
  ...
  # filter(!(sample_origin == "normal" & str_detect(sub_cell_type, "CAFs"))) %>% 
  # slice(1:3) %>% 
  ungroup %>% 
...
ligands_imm <- ligand_receptor_summary %>% 
  ...
  # slice(1:3) %>% 
  ungroup %>% 
  mutate(sub_cell_type = ordered(sub_cell_type, levels = levels(seu_imm_pri$cell_type_imm_simple)))
...
receptors_epi <- ligand_receptor_summary %>% 
  ...
  filter(str_detect(sub_cell_type, "TA|TC"))
...
str_vs_epi
ggsave("_fig/_patients/ligand_receptor_connectivity_str_vs_epi_scaled_sub_grey.pdf", width = 4.5, height = 8)
...
imm_vs_epi
ggsave("_fig/_patients/ligand_receptor_connectivity_imm_vs_epi_scaled_sub_grey.pdf", width = 4, height = 8)
